...
| 4b | **NEW** What we said before each race (`ledger`) | open | Per `docs/LEDGER_SPEC.md` §4. `C_LED_1` as Section caption. Visible captioned DataTable (columns Round · Preview dated (`computed_at`) · Our top three (p10–p90 beside each) · Actual top three · Rank agreement · Claimed · Bands held · Claimed · Not classified), one row per round whose scored snapshot is the greatest `computed_at` strictly before the race session's `start_utc`; result columns a dash until results exist. Then `cLed2(races, first)` and `C_LED_3`. Empty states through `emptyReason`: `C_LED_EMPTY_NONE` (no snapshot rows), else `cLedEmpty(previews)` (no scored round). Cut if late: title-odds table `title-record` (top five by `p_title` at the latest `after_round`, columns `after_round` 1..N, range beneath each figure, "prior" badge when `is_shrunk_to_prior`) with caption `C_LED_4` — a nightly reconstruction, not a publication record. |
